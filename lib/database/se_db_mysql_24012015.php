...
define('DB_CACHED', true); // Р’РєР»СЋС‡РёС‚СЊ РєСЌС€РёСЂРѕРІР°РЅРёРµ

function se_db_connect($link = 'db_link')
{
  global $$link, $CONFIG, $MEM;
  if ($link == "db_link")
  {
    if (!empty($CONFIG)){
...
function se_db_is_field($table, $field, $link = 'db_link')
{
  ...
  return (!empty($aresult)) ? mysql_num_rows($aresult) : false;
}

function se_db_is_index($table, $field_name, $name_index = ''){
    global $$link;
    $key_index = ($name_index) ? " AND `Key_name`='{$name_index}'" : '';
    $aresult = mysql_query("SHOW INDEX FROM `{$table}` WHERE `Column_name` = '{$field_name}'".$key_index);
    $ares = (!empty($aresult)) ? mysql_num_rows($aresult) : false;
    return $ares;
