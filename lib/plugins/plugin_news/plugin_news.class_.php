...
class plugin_news
{
    ...
    public function getItem($id)
    {
        ...
        GROUP_CONCAT(ngc.id_gcontact) AS gcontacts");
        $news->leftjoin("news_gcontacts ngc", "`n`.id = `ngc`.id_news");
        $news->leftjoin("news_userfields nuf", "`n`.id = `nuf`.id_news");
        $val = $news->find($id);
        if ($this->id_gcontact && $val['gcontacts']) {
    ...
    public function getItems($code = '', $offset = 0, $limit = 30)
    {
        ...
        foreach (self::$sections as $item) {
            if ($this->id_gcontact && $item['gcontacts']) {
    ...
    public function getList($code = false, $limit = 30)
    {
        ...
        $news->andWhere("((SELECT COUNT(ngc1.*) FROM news_gcontacts ngc1 WHERE `ngc1`.id_news=`n`.id) < 1 OR ngc.id_gcontact=?)", $this->id_gcontact);
        $news->groupBy('n.id');
        $news->orderBy('sort', 0);
        $news->addOrderBy('news_date', 1);
