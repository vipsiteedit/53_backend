...
class plugin_shopdelivery
{
	...
	private $service_settings = array(
		'sdek' => array(
            ...
            'map_key' => array(
				'name' => 'API ключ карты',
				'description' => 'Укажите API ключ Яндекс.Карты для отображения пунктов выдачи на карте',
				'code' => 'map_key',
				'type' => 'text',
				'value' => null,
			),
    ...
    public function __construct($not_delivery = array())
    {
        ...
        if (isRequest('delivery_sub_' . $this->delivery_type_id))
            $_SESSION['delivery_sub'][$this->delivery_type_id] = getRequest('delivery_sub_' . $this->delivery_type_id, 3);
        ...
        if (!empty($deliveries)) {
            ...
            foreach ($deliveries as $key => $delivery) {
                ...
                if ($delivery['code'] == 'region' || $delivery['code'] == 'subregion') {
                    ...
                    $delivery_post = $this->getDeliveryPricePost($delivery['id']);
                    ...
                    if (empty($delivery_cdek)) {
                        ...
                        $delivery['note'] .= $delivery_cdek['note'];
						$delivery['name'] .= $delivery_cdek['name'];
                    }
    ...
    private function getDeliveryPricePost($id = 0)
    {
        ...
        if (!empty($settings['from_index']['value'])) {
            $from_index = $settings['from_index']['value'];
        }
                    ...
                    if (!empty($data['result'])) {
						...
						if (!empty($get_points)) {
                            ...
                            foreach ($points as &$point) {
                                $point['price'] = $delivery['price'];
                                $point['time'] = $delivery['time'];
                                if ($point['sel']) {
                                    $delivery['name'] = ' (' . $point['name'] . ')';
                                }
                            }
                            
                            if ($settings['map_key']['value'] && $this->delivery_type_id == $id) {
                            
                                $delivery['note'] .= '<style>.deliveryType[data-id="'.$id.'"] .subdeliveries{display:none !important;}</style>';
                                
                                $delivery['note'] .= '<script src="https://api-maps.yandex.ru/2.1/?lang=ru_RU&apikey=' . $settings['map_key']['value'] . '" type="text/javascript"></script>';
                                
                                $delivery['note'] .= '<div id="map" style="width:100%; height:370px"></div>';
                                
                                $delivery['note'] .= '<script type="text/javascript">
    ymaps.ready(init);
 
    function init() {
        $(".deliveryType[data-id='.$id.'] .subdeliveries").hide();
        ...
        $.each(points, function(i, point){
            var button = $("<button class=\"btn btn-select-point\" data-id=\""+point.id+"\" type=\"button\">Выбрать</button>");
            if (point.sel) {
                button.addClass("disabled").text("Выбран");
            }
            var myPlacemark = new ymaps.Placemark([point.latitude, point.longitude], 
                {
                    hintContent: point.name,
                    balloonContent: "<strong>" + point.name + "</strong>" + point.note + button.prop("outerHTML"),
                    iconContent: i + 1 
                },
                {
            ...
            myPoints.push({coords: [point.latitude, point.longitude], text: point.name + ", " + point.addr, address: point.addr});
        });
    ...
    CustomSearchProvider.prototype.geocode = function (request, options) {
        ...
        for (var i = 0, l = points.length; i < l; i++) {
            ...
            geoObjects.add(new ymaps.Placemark(point.coords, {
                name: point.text,
                description: "",
                balloonContentBody: "<p>" + point.text + "</p>",
                boundedBy: [point.coords, point.coords]
            }));
        }
    ...
    $("#map").on("click", ".btn-select-point", function(e){
        e.preventDefault();
        $("input[value=\'"+$(this).data("id")+"\']").click();
    })
			...
			if ($status == 200 && $response !== false) {
				...
				if ($xml->Pvz) {
					foreach ($xml->Pvz as $pvz) {
						...
						$description = '<p><i title="' . (string)$pvz['FullAddress'] . '" target="_blank">' . (string)$pvz['Address'] . '</i>, ' . (string)$pvz['Note'] . ', Тел.:' . (string)$pvz['Phone'] . '</p>';
						
						$description .= '<p>Время работы: ' . (string)$pvz['WorkTime'] . '</p>';
						
						$id = $id_sub . ':' . $id_delivery;
						...
						$points[] = array(
							...
							'note' => $description,
                            'addr' => (string)$pvz['Address'],
                            'address' => (string)$pvz['FullAddress'],
							'sel' => ($_SESSION['delivery_sub'][$id_delivery] && $_SESSION['delivery_sub'][$id_delivery] == $id),
                            'latitude' => (string)$pvz['coordY'],
                            'longitude' => (string)$pvz['coordX'],
						);
        ...
        if ($id) {
            ...
            if ($delivery_type->isFind()) {
                ...
                if ($delivery['code'] == 'region') {
                    ...
                    $delivery['time'] = $delivery_ems['time'];
                } elseif ($delivery['code'] == 'post' && $delivery_post = $this->getDeliveryPricePost($id)) {
