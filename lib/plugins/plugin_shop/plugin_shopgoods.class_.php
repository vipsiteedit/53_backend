...
class plugin_shopgoods {
        ...
        if (!empty($id_goods)) {
            ...
            if (!empty($featurelist)) {
                foreach($featurelist as $val) {
                    ...
                    if (!empty($val['gimage'])) {
						$val['gimage'] = '/images/' . $this->lang . '/shopfeature/' . $val['gimage'];
					...
					if (!empty($val['fimage'])) {
						$val['fimage'] = '/images/' . $this->lang . '/shopfeature/' . $val['fimage'];
  ...
  /*  public function getGoods($options = array(), $goods_name = '') {
	    ...
	    $price = new seTable();
        $price->setSql($this->getGoodsSql($options, $goods_name));
		$price->setListCount($this->getGoodsSql($options, $goods_name), true);
		$SE_NAVIGATOR = $price->pageNavigator($options['limit']);
		$pricelist = $price->getList();
        $goodscount = count($pricelist);
        // сортировка случайным образом
        if (($option['sort'][0]=='R')&&($options['special'])) { 
            if (!empty($pricelist)) {
                $dataarr = array();
  ...
  /** Получить id стартовой группы 
    ...
    **/
    public function showGoodsDescription($goods_id='') {
		...
		$shop_price->select('id_group, title, name, keywords, description, img, img_alt, article, note, text, manufacturer, measure, price, discount');
