...
class plugin_shopDelivery40
{
	...
	public function __construct($not_delivery = array()) 
	{
		...
		$plugin_cart = new plugin_ShopCart40(array('curr' => se_baseCurrency()));
	...
	public function getDeliveryList()
	{
		...
		$rdeliv->andWhere("(sdt.max_weight >= '?' OR sdt.max_weight IS NULL)", floatval($this->total['weight']));
		$rdeliv->andWhere("(sdt.max_volume >= '?' OR sdt.max_volume IS NULL)", floatval($this->total['volume']));
		...
		if (!empty($deliveries)){
			...
			foreach($deliveries as $key => $delivery){
				...
				elseif ($delivery['code'] == 'EMS'){
					$delivery_ems = $this->getDeliveryPriceEms();
