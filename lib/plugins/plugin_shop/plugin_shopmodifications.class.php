...
class plugin_shopmodifications {
	...
	public function getAllList($in_sql = null) {
        ...
        $shop_modifications->andwhere('smf.id_price = ?', $this->id_price);
		if (!empty($in_sql))
        ...
        $shop_modifications->groupBy('sm.id, sf.id'); 
	...
	public function getChangesModification($options = array()) {
		...
		$response['presence'] = (string)$plugin_amount->showPresenceCount($options['not_stock'], $options['in_stock']);
		$response['price'] = array(
			'new' => $plugin_amount->showPrice(true, $options['round']),
			'old' => $plugin_amount->showPrice(false, $options['round']) 
		);

		if (!empty($options['images']))
			$response['images'] = $this->getImages($selected);
        
        $response['description'] = $this->getdescription($selected);
    ...
    public static function getDescription($modifications)
    {
		...
		if (!empty($modifications)) {
			...
			$sm = new seTable('shop_modifications', 'sm');
			$sm->select('sm.description, sp.note');
			$sm->where('sm.id=?', trim(array_shift($modifications)));
            $sm->innerJoin('shop_price sp', 'sm.id_price=sp.id');
			$sm->fetchOne();
			if ($sm->isFind()) {
				$description = $sm->description ? $sm->description : $sm->note;
            }
		}
