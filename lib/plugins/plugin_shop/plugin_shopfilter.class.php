...
class plugin_shopfilter {
                        ...
                        if (isset($value['from']) && isset($value['to'])) {
                            ...
                            if (!empty($value['from']) || !empty($value['to'])) {
                                ...
                                elseif (!empty($value['from'])) 
                                    $where[] = '('.$spf.'.id_feature='.$key.' AND '.$spf.'.value_number >= "' . (float)$value['from'] . '")';
                                elseif (!empty($value['to'])) 
                                    $where[] = '('.$spf.'.id_feature='.$key.' AND '.$spf.'.value_number <= "' . (float)$value['to'] . '")';
                            }
                        } 
                        else {
                            $join[] = array('type'=>'inner', 'table'=>'shop_modifications_feature ' . $spf, 'on'=>"$spf.id_price=sp.id");
    ...
    public function getFilterValues($tree_group = null, $flLive = false) {
		...
		se_db_query('SET group_concat_max_len = 8096;');
        ...
        $price_feature->select("
            ...
            GROUP_CONCAT(DISTINCT CASE
                WHEN (sf.type = 'list' OR sf.type = 'colorlist') THEN (SELECT CONCAT_WS('##', RIGHT(CONCAT('0000', sfv.sort), 5), sfv.value, sfv.id, CONCAT_WS('', sfv.color, ''), sfv.image) FROM shop_feature_value_list sfv WHERE sfv.id = spf.id_value LIMIT 1)
		...
		$price_feature->innerJoin('shop_price_group spg', 'sp.id=spg.id_price');
        $price_feature->where("(sp.enabled='Y')");
		
		$price_feature->innerJoin('shop_group_tree sgt', 'spg.id_group=sgt.id_child');
		$price_feature->andWhere('sgt.id_parent=?', $this->group);
        ...
        if (isset($filter_list['brand'])) {
            $shop_price = new seTable('shop_brand', 'sb');
            $shop_price->select('DISTINCT sb.id, sb.name');
            $shop_price->innerjoin('shop_price sp', 'sb.id=sp.id_brand');
			if ($flLive) {
				foreach($join as $jn) {
			...
			$shop_price->where('sp.enabled = "Y"');
			$shop_price->andWhere('sgt.id_parent=?', $this->group);
            if ($shop_price->isFindField('sort'))
                $shop_price->addOrderby('sb.sort');
			$shop_price->addOrderby('sb.name', 0);
