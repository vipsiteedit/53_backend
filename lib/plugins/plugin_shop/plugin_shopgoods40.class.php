...
class plugin_shopgoods40 {
	...
	**/ 
	private function setRequest(){
           ...
           if (get('pricemoney')) {
                $this->pricemoney = $_SESSION[SE_DIR . 'pricemoney'] = get('pricemoney',0);
           } elseif (empty($_SESSION[SE_DIR . 'pricemoney'])) {
                $this->pricemoney = $_SESSION[SE_DIR . 'pricemoney'] = $basecurr = se_baseCurrency();
           } else {
                $this->pricemoney = $_SESSION[SE_DIR . 'pricemoney'];
           }
	}
    ...
    **/    
    public function getGoods($options=array(), $goods_name = '') {
		...
		if ($options['is_base_group']) $this->Group_id = $this->baseGroupId;
    ...
    **/
    public function getRequestGroup($old_format=false, $group_name = ''){
        ...
        if (get('shopcatgr')) {
            ...
            if (get('cat'))
                $group_name = urldecode(get('cat',3)); 
            if ($group_name)
  ...
  /** Получить id стартовой группы 
    ...
    **/
    public function showGoodsDescription($goods_id=''){
        if($goods_id=='') return;
        $show = new seTable('shop_price', 'sp');
        ...
        $show->select('sp.*,'.$this->getBrandSql('sp.id'));
        $show->where('sp.id=?', $goods_id);
		$goods = $show->fetchOne();
        //проверка на существование и на статус активный товара
        if(!$show->isFind() || ($goods['enabled'] == 'N') || (!$goods['enabled'])){
    ...
    **/
    ...
    public function setUserLooks($id_price, $expire = '-1'){
  		 if (!$id_price) return;
			 ...
			 if ($_COOKIE["look_goods"][$id_price]){
