...
class plugin_shopgroups {
    ...
    public function __construct()
    {
        ...
        $this->cache_images = $this->cache_dir . 'images.json';
		$this->cache_img_count = $this->cache_dir . 'images_count.txt';
	...
	*/
	...
	private function parseTreeGroups($upid = 0, $level = 0)
    {
        ...
        if ($sg->isFindField('is_market'))
            $sg->addSelect('sg.is_market');
		...
		if (!empty($list)) {
			foreach ($list as $val) {
				...
				$this->groups[$val['id']] = array(
                    ...
                    'is_market' => isset($val['is_market']) ? $val['is_market'] : 1,
					'children' => array(),
					'childrens' => array(),
					'parent' => $upid,
					'parents' => array(),
    ...
    private function parseGroupImages()
    {
		$this->images = array();
		
		$result = se_db_query('SELECT COUNT(*) AS count, UNIX_TIMESTAMP(GREATEST(MAX(updated_at), MAX(created_at))) AS time FROM shop_group_img');
		
		list($count, $time) = se_db_fetch_row($result);
		
		$cache_count = file_exists($this->cache_img_count) ? (int)file_get_contents($this->cache_img_count) : -1;
		
		$time = max(filemtime(__FILE__), $time);
		
		if (file_exists($this->cache_images) && filemtime($this->cache_images) > $time && $cache_count == $count) {
			$this->images = json_decode(file_get_contents($this->cache_images), 1);
		}
		else {
			$sgi = new seTable('shop_group_img');
			$sgi->select('id, id_group, picture, picture_alt, title');
			$sgi->orderBy('sort');
			$list = $sgi->getList();
			if ($list) {
				foreach ($list as $val) {
					$this->images[$val['id_group']][] = array(
						'id' => $val['id'],
                        'image' => $val['picture'],
						'alt' => $val['picture_alt'],
						'title' => $val['title'],
			...
			$file = fopen($this->cache_images, "w+");
			$result = fwrite($file, json_encode($this->images));
			fclose($file);
			
			$this->writeLog($this->cache_images . ' - ' . $result);
			
			$file = fopen($this->cache_img_count, "w+");
			$result = fwrite($file, $count);
			fclose($file);	
		}
	}
	...
	public function getImages($id)
    {
		if (is_null($this->images)) 
			$this->parseGroupImages();
		$id = $this->getId($id);
		if (empty($this->images[$id])) return;
