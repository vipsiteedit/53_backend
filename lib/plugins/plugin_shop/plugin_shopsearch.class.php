...
class plugin_shopsearch {
	...
	public function getProducts($options) {
		...
		if (!empty($query)){
			...
			$shop_price->select('DISTINCT sp.id, sp.name, sp.code, sp.article, sp.presence_count, sp.step_count, sp.price, sp.price_opt, sp.price_opt_corp, sp.discount, sp.max_discount, sp.curr, sp.id_group, (SELECT 1 FROM shop_modifications sm WHERE sm.id_price=sp.id LIMIT 1) AS modifications, (SELECT si.picture FROM shop_img si WHERE si.id_price = sp.id ORDER BY si.`default` DESC, si.sort ASC LIMIT 1) AS img');
			...
			foreach($words as $word){
				$search_query = 'sp.name LIKE "%?%"';
				if ($options['search_article'])
					$search_query .= ' OR sp.article LIKE "%?%"';
			...
			if (!empty($goods_list)) {
				...
				$link = seMultiDir() . '/' . $this->page . '/show/';
				foreach($goods_list as $val) {
					$goods = array();
					$goods['url'] = $link . $val['code'];
