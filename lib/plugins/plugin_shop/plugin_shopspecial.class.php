...
class plugin_shopspecial
{
    ...
    {
        $select_list = 'DISTINCT sp.id, sp.code, sp.id_group, sp.flag_hit, sp.flag_new, (SELECT 1 FROM shop_modifications sm WHERE sm.id_price=sp.id LIMIT 1) AS modifications';
        ...
        if (!empty($options['field_label'])){
            $select_list .= ', (SELECT GROUP_CONCAT(t.id_label) FROM shop_label_product AS t WHERE t.id_product=sp.id) AS labels';
        }
        ...
        switch ($this->selection){
            ...
            case 'label':
                ...
                $shop_price->innerJoin('shop_label_product slp', 'sp.id=slp.id_product');
                $shop_price->andWhere('slp.id_label=?', $p->getLabelId($options['label']));
                break;
        }
        ...
        if (!$id_list) {
            $shop_price->orderBy('sp.' . $sort_field, $sort_dir);   
        }
    ...
    private function eachSpecial($list, $options)
    {
        if (!empty($list)) {
            ...
            foreach($list as $key => $line) {
                ...
                if ($line['modifications'] && empty($_SESSION['modifications'][$line['id']])) {
                ...
                if (!empty($options['field_group_name']))
                    $line['view_group'] = seMultiDir() . '/' . $options['page_shop'] . '/cat/' . urlencode($line['code_gr']);
                
                $line['view_goods'] = seMultiDir() . '/' . $options['page_shop'] . '/show/' . urlencode($line['code']);
                
                $line['price_label'] = $this->getPriceLabel($line['id'], $options['price_label']);
                
                if (!empty($line['labels'])) {
                    $id = explode(',', $line['labels']);
                    $line['labels'] = array();
                    $p = plugin_shoplabel::getInstance();
                    foreach ($id as $val) {
                        $line['labels'][] = $p->getLabel($val);
                    }
                }
