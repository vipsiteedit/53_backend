            ...
            if (!empty($featurelist)) {
				...
				foreach($featurelist as $val) {
					...
					if ($val['type'] == 'colorlist' || $val['type'] == 'list') {
						list($val['value'], $color, $image) = explode('##', $val['value']);
					}
                    $feature = array(
                        'id' => $val['fid'],
                        'type' => $val['type'],
...
//echo 'update';
		...
		if (empty($goods_name)) {
			...
			if (isRequest('q', 1)) {
				...
				$price->andWhere("(sp.name LIKE '%?%' OR sp.article LIKE '%?%')", $query);
			}
  ...
  /** Получить id стартовой группы 
    ...
    **/
	...
	public static function getGoodsId($code = '') {
		if (empty($code))
    ...
    public function viewgalleryImages($price_id, $size_prev, $size_mid, $size_full, $res = 's', $watermark = '') {
		...
		if(!empty($imglist)) {
			foreach($imglist as $val) {
				$image = array();
				if($val['picture']!='') {
					list($image['image'], $nofoto) = $this->getGoodsImage($val['picture'], $size_full, $res, $watermark);
					if ($nofoto) continue;
					$image['title'] = $val['title'];
					$image['alt'] = $val['picture_alt'];
					$image['id'] = $val['id'];
					list($image['image_prev'],) = $this->getGoodsImage($val['picture'], $size_prev, $res);
					list($image['image_mid'],) = $this->getGoodsImage($val['picture'], $size_mid, $res, $watermark);
					$images[] = $image;
				}
			}  
        }
