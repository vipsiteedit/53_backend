	...
	*/
	public function execute($indelivery = array(), $email = '', $param = array(), $discount = 0, $coupon = null, $save_fields = false) {
		...
		if (!empty($this->incart)) {
				...
				if ($order_id > 0) {
					...
					if (!empty($this->indelivery)) {
						...
						if (!empty($_SESSION['delivery_sub'][$this->indelivery['id']])) {
							...
							$id_subdelivery = $_SESSION['delivery_sub'][$this->indelivery['id']];
							
							if (is_numeric($id_subdelivery )) {
								$delivery->id_subdelivery = $id_subdelivery;
							}
							else {
								$id_delivery = $this->indelivery['id'];
								$plugin_delivery = new plugin_shopdelivery();
								$deliverytype = $plugin_delivery->getDelivery($id_delivery);
								
								if (!empty($deliverytype['sub'])) {
									foreach ($deliverytype['sub'] as $sub) {
										if ($sub['id'] == $id_subdelivery || $sub['id_orig'] == $id_subdelivery) {
											$this->indelivery['address'] = $sub['address'];
											break;
										}
									}
								}
            ...
            if ($this->indelivery['id']) {
				$delivery = new seTable('shop_deliverytype');
				$delivery->select('id, email');
				$delivery->find($this->indelivery['id']);
				
                if ($delivery->email) {
					$mails->sendmail('orderprovider', $delivery->email);
				}
			}
