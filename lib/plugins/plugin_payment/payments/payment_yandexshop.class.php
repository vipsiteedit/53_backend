...
class payment_yandexshop extends basePayment{
    ...
    {
        return array('shopId'=>'ID магазина', 'scid'=>'Номер витрины','wmpassw'=>'секретный код', 'taxSystem'=>'Система налогообложения магазина');
    }
    ...
    {
        $macros = new plugin_macros(0, $this->order_id, $this->payment_id);
        $url = ($this->test) ? 'https://demomoney.yandex.ru/eshop.xml' : 'https://yoomoney.ru/eshop.xml';
        //$url = "https://demomoney.yandex.ru/eshop.xml";
        
        $shopId = $macros->execute('[PAYMENT.SHOPID]');
        $scid = $macros->execute('[PAYMENT.SCID]');
        $taxSystem = $macros->execute('[PAYMENT.TAXSYSTEM]');
        ...
        $receipt = array(
            ...
            'taxSystem' => $taxSystem ? $taxSystem : 1,
            'items' => array(),
        );
        ...
        foreach ($list as $val) {
            $receipt['items'][] = array(
                'quantity' => $val['count'],
                'price' => array('amount' => round($val['price'] - $val['discount'], 2)),
                'tax' => 4,
                'text' => $val['nameitem'],
            );
        }
        ...
        if ($delivery_name = $macros->execute("[ORDER.DELIVERY_NAME]")) {
            $price_delivery = $macros->execute("[ORDER_DELIVERY]");
            
            $receipt['items'][] = array(
                'quantity' => 1,
                'price' => array('amount' => round($price_delivery, 2)),
                'tax' => 1,
                'text' => 'Доставка заказа ' . $delivery_name,
            );
        }

        $blank = "<h4 class=\"contentTitle\">Подтверждение оплаты через Ю.Касса</h4>
                    <div class=''>Вы будете переведены на сайт платежной системы Ю.Касса для оплаты заказа № $order_id</div>";
        $blank .= '[SETCURRENCY:RUR]<form method="POST" action="'.$url.'">
                        ...
                        <input name="ym_merchant_receipt" value=\'{ym_merchant_receipt}\' type="hidden"/>
                        <input type="submit" name="submit-button" value="Оплатить">
                    </form>';

        $this->newPaymentLog();
        
        $json_receipt = preg_replace_callback('/\\\\u(\w{4})/', function ($matches) {
            return html_entity_decode('&#x' . $matches[1] . ';', ENT_COMPAT, 'UTF-8');
        }, json_encode($receipt));
        
        $blank = str_replace('{ym_merchant_receipt}', $json_receipt, $macros->execute($blank));
