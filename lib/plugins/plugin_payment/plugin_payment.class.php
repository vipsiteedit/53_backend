...
class plugin_payment {
	...
	public function __construct($order_id = 0, $payment_id = 0, $is_test = 0)
	{
		...
		if (trim($payment['ident']) != '') {
			if (file_exists(SE_ROOT . 'system/plugins/payment_' . $payment['ident'].'.class.php')) {
				require_once(SE_ROOT . 'system/plugins/payment_' . $payment['ident'].'.class.php');
				$plugin = 'payment_' . $payment['ident'];
				
				$result = new $plugin($order_id, $payment_id);
			}
            elseif (file_exists(dirname(__FILE__).'/payments/payment_'.$payment['ident'].'.class.php')) {
