...
class plugin_macros {
    ...
    private function parseOrder($text){
        if ($this->order_id) {
            ...
            $array_change = array('ORDER_DISCOUNT' => se_formatMoney($discount, $this->curr),
                ...
                'ORDER.SUMMA_TOTAL'=>str_replace(',','.', round($summ - $discount, 2)),
            ...
            if (!empty($fields_list)) {
                ...
                foreach ($fields_list as $key => $value)
                    $table_fields .= "<tr><td>{$key}</td><td>{$value}</td></tr>";
                $table_fields .= '</table>';
            ...
            if (preg_match("/\<SHOPLIST\>([\w\W]{1,})\<\/SHOPLIST\>/i", $text, $res_math) || preg_match("/\<!--\<SHOPLIST\>--\>([\w\W]{1,})\<!--\<\/SHOPLIST\>--\>/i", $text, $res_math)){
                $SHOPLIST="";
                $query=se_db_query("SELECT sp.id, sp.name, sp.article, st.`nameitem`, st.count,st.discount,st.price, (SELECT `picture` FROM shop_img WHERE id_price=sp.id AND `default`=1 LIMIT 1) AS img
                    FROM shop_tovarorder st
                    LEFT OUTER JOIN shop_price sp ON (sp.id = st.id_price)
                    WHERE (`id_order`='{$this->order_id}')");
                $it = 0;
                while ($res=se_db_fetch_assoc($query)){
                    ...
                    if (strpos($text, '[SHOPLIST.FEATURE') !== false) {
                        $f = new seTable('shop_modifications_feature', 'smf');
                        $f->select('
                            sf.id,
                            sf.name,
                            GROUP_CONCAT(CASE    
                        ...
                        ');
                        $f->innerJoin('shop_feature sf', 'sf.id=smf.id_feature');
                        $f->where('smf.id_price=?', $res['id']);
                        $f->groupBy('sf.id');
                        $list = $f->getList();
                        
                        foreach ($list as $val) {
                            $value = $val['value'];
                            $res['feature['.$val['id'].']'] = $value;
                        }
                    }
    ...
    private function parsePayment($text) {
        if (strpos($text, '[PAYMENT.')!==false) {
            ...
            $query = se_db_query("select codename,value FROM bank_accounts WHERE id_payment={$this->payment_id};");
            if (!empty($query))
