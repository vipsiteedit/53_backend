<?php
function module_mshop_vitrine53($razdel, $section = null)
{
 ...
 if (strpos(dirname(__FILE__),'/lib/modules'))
   $__MDL_URL = 'lib/modules/mshop_vitrine53';
 else $__MDL_URL = 'modules/mshop_vitrine53';
 $__MDL_ROOT = dirname(__FILE__).'/mshop_vitrine53';
 ...
 if (isRequest('addcart') && empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {       
     $shopcart = new plugin_shopcart53();
     $shopcart->addCart();                                  
 ...
 $psg = new plugin_shopgoods53($virtualpage, '', trim($section->parametrs->param273)); 
 ...
 if ((isRequest('ajax'.$razdel)) && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
     ...
     if (isRequest('param')) {
         ...
         $plugin_amount = new plugin_shopamount53($id_price, '', $price_type, 1, $selected);
