var ashop_cart_compl_execute = function(params){ 
...
var insert = '<tr class="tableRow itemCart updateAjax" data-id="' + data.id +'">' +
    ...
    '<td class="itemCountCart"><div class="cartitem_count"><a href="#" class="b_shop_cart-cartitem_count_button decCountItem" data-action="decrement">-</a> <input class="cartitem_inputcn" type="text" name="countitem[' + data.id + ']" value="' + data.count + '" size="3"> <a href="#" class="b_shop_cart-cartitem_count_button incCountItem" data-action="increment">+</a></div></td>' +
    '<td class="itemSumCart"><span class="summBlock">' + data.sum + '</span></td>' +
    '<td class="itemDeleteCart"><a href="' + params.link + 'delcartname/' + data.id + '/" class="b_shop_cart-button_delete_shop_item btnDeleteItem" title="Удалить товар из корзины">&nbsp;</a></td></tr>';
...
$(document).ready(function () {
    ...
    $('.paymentType .linkShowNote').each(function(){
        var note = $(this).closest('.paymentType').find('.paymentNote');
        $(this).tipso({
            background : '#313131',
            width : 400,
            position : 'bottom',
    ...
    $('.b_shop_cart-cartitem_count_button').show();
    ...
    $('.tableListGoods').on('click', '.b_shop_cart-cartitem_count_button', function(){
