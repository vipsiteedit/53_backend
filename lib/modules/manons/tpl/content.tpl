            ...
            <?php foreach($__data->limitObjects($section, $section->objectcount) as $record): ?>
                ...
                <div class="object record-item" <?php echo $__data->editItemRecord($section->id, $record->id) ?>><?php echo $__data->linkEditRecord($section->id, $record->id,'') ?>
                   ...
                   <?php if(!empty($record->image)): ?>
                        <a class="record-image-link" href=<?php if($record->field!=''): ?>"<?php echo $record->field ?>"<?php else: ?>"#"<?php endif; ?>>
                            <img class="objectImage record-pimage" src="<?php echo $record->image_prev ?>" border="0" alt="<?php echo $record->image_alt ?>">
                        </a>
                    <?php endif; ?>
