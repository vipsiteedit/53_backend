    ...
    <script>
        $(function($){
            if (typeof $.fn.inputmask == 'function')
                $(".input_phone").inputmask("<?php echo $section->parametrs->param15 ?> (999) 999-99-99");
        });
        $('#feedback-form<?php echo $section->id ?>').off().on('submit', function(e){
            e.preventDefault();
            var form = $(this);
            $.ajax({
                url: '<?php echo seMultiDir()."/".$_page."/" ?>?part=<?php echo $section->id ?>',
                type: 'post',
                data: form.serialize(),
                success: function(data){
                    <?php if(strval($section->parametrs->param20)=='Y'): ?>
                        form.closest('.modal_block').html('<div class="alert_mes">' + data +', <?php echo $section->language->lang010 ?>' + '</div>');
                    <?php else: ?>
                        alert(data + ', <?php echo $section->language->lang010 ?>');
                    <?php endif; ?>
                    form.trigger('reset');
                },
                error: function(){
...
<div class="content gs_form_mod" <?php echo $section->style ?>>
    <?php if(strval($section->parametrs->param20)=='Y'): ?>
        <a id="fancybox<?php echo $section->id ?>" class="link_mod" href="#modal<?php echo $section->id ?>"><?php echo $section->parametrs->param17 ?></a>
    ...
    <div id="modal<?php echo $section->id ?>" class="modal_block"> 
        ...
        <div class="contentBody">  
            <form method="POST" class="fform" id="feedback-form<?php echo $section->id ?>">
                ...
                <div class="blockBtn">
                    <input class="buttonForm buttonSend" type="submit" <?php if(strval($section->parametrs->param12)=='Y'): ?>onclick="ym(<?php echo $section->parametrs->param10 ?>,'reachGoal','<?php echo $section->parametrs->param11 ?>');"<?php endif; ?> value="<?php echo $section->language->lang009 ?>">
                </div>
            </form>
        </div>
