...
function module_ashop_cart53($razdel, $section = null)
{
 ...
 if (!empty($confirm_order)) {
     ...
     $order_id = $plugin_order->execute($order_delivery, $_SESSION['cartcontact']['email'], '', $order_discount, $coupon_used, $section->parametrs->param19 == 'Y');
     ...
     if (!empty($_SESSION['payment_type_id']) && $section->parametrs->param9 == 'Y'){ 
         ...
         $redirect = seMultiDir().'/'.$section->parametrs->param2.'/invnum/'.$payment_id.'_'.$order_id.'/'.md5($payment_id.'_'.$order_id.'3dfgvj').'/';
     }
     else{
         $redirect = seMultiDir().'/'.$section->parametrs->param2.'/?'.time();    
     }
