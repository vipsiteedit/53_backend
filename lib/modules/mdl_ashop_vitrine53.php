...
function module_ashop_vitrine53($razdel, $section = null)
{
 ...
 $is_lazy = ($section->parametrs->param346=='Y');   // Ленивая загрузка фото товаров
 $user_id = seUserId();
 ...
 if ((isRequest('ajax'.$razdel)) && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
     ...
     elseif (isRequest('add_wish')){
            
          $id_price_wish = getRequest('add_wish');
          $response['id'] = $id_price_wish;
          $response['id_user'] = $user_id;
          
          if(!empty($id_price_wish) && $user_id!=0){
    
             $is_wish = $psg->getGoodsWish($user_id, $id_price_wish);
      
             if($section->parametrs->param347 == 'Y'){
                 if($is_wish == true){
                     $response['active'] = true;
                     if(se_db_query("DELETE FROM shop_wishlist WHERE id_user = {$user_id} AND id_price = {$id_price_wish}") == TRUE){
                     ...
                     if(se_db_query("INSERT INTO shop_wishlist(id_user, id_price) VALUES ({$user_id}, {$id_price_wish})") == TRUE){
