<?php
function module_mshop_flycart53($razdel, $section = null)
{
 ...
 if (strpos(dirname(__FILE__),'/lib/modules'))
   $__MDL_URL = 'lib/modules/mshop_flycart53';
 else $__MDL_URL = 'modules/mshop_flycart53';
 $__MDL_ROOT = dirname(__FILE__).'/mshop_flycart53';
 ...
 $plugin_cart = new plugin_shopcart53(array('round' => $rounded, 'type_price' => $type_price, 'presence' => '', 'curr' => $pricemoney));
 ...
 if (isRequest('ajax'.$razdel) && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
     ...
     elseif (getRequest('addcart', 1) > 0){
         ...
         $result['add'] = $plugin_cart->addCart();
