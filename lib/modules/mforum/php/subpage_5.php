    ...
    if (isRequest('upload')) {
        ...
        if ($s['id_users'] == $uid) {
            ...
            if (!empty($userfile)) {
                ...
                if (!(preg_match ("/^[^\t]+\.(gif|jpg|png)$/u", $origfilename) && ($mimetype==1 || $mimetype==2 || $mimetype==3))) {
                    $error = "{$section->language->lang110} GIF/JPG/PNG!";
                    $error_exists = 1;
                } else {
//                    $extimg = utf8_substr($origfilename, 3);
                    ...
                    if (rename (getcwd () . "/modules/forum/images/" . $previmg, 
                                    $uploadfile)) {
