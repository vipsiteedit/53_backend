    ...
    if (!empty($_SESSION['forum_attached'])) {
        foreach ($_SESSION['forum_attached'] as $k => $v) {
            if ($v['id'] == $delfile) {
                ...
                if (preg_match("/\.(gif|jpg|png)$/u", $filename)) {
...
if (isRequest('upload')) {
    ...
    $userfname = str_replace(']', '_', $userfname);
//    if (!(preg_math("/^[^\.]+\.(gif|jpg|png|rar|zip|arj|gz)$/u", $userfname))) {
    
    $ext_list = explode(',', $section->parametrs->param100);
    
    foreach ($ext_list as &$val) {
        $val = trim($val);
    }
    
    if (!(preg_match("/\.(".join('|', $ext_list).")$/i", utf8_strtolower($userfname)))) {
        $error = "{$section->language->lang099} " . join(', ', $ext_list);
        $error_exists = 1;
    } else {
        ...
        if (count($forum_attached) >= $maxFilesAttached) {
            ...
            //----------------------------------------------------/\
                    ...
                    if (move_uploaded_file($userfile, $uploadfile)) {
                        ...
                        $_SESSION['forum_attached'] = $forum_attached;
                        if (preg_match("/^.+\.(gif|jpg|png)$/u", utf8_strtolower($uploadfile))) {
        ...
        if (!$error_exists) {
            ...
            if ($quote_id) {
                ...
                $qtext = textedit ($qtext, $q_id_users, $quote_id, 
                                $iconssmiles, $_page, $razdel, false, $section); 
                ...
                foreach($forum_attached as $af) {
                    $__data->setItemList($section, 'fatt', 
                        array(
                                ...
                                'id' => $af['id'],
                                'img' => intval(preg_match("/^.+\.(gif|jpg|png)$/u", $af['name']) != 0),
 ...
 //Посылаем сообщение автору
                if (!$newt) {
                    ...
                    if (!empty($topic['email'])) {
                        ...
                        $text = textedit ($text, $realid, $msgid, $iconssmiles, $_page, $razdel, true, $section); 
                        $mail_text .= $text;
