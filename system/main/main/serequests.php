...
function from_Url($REQUEST = '')
{
    ...
    if (is_array($se_fromurl) && $global) { //URI уже был разобран, выдаем глобальную переменную
        ...
        */
        ...
        if (!empty($res[$i]) && preg_match("/^([\d]{1,})$/im", $res[$i])) {
            if (!empty($res[$i+1]) && preg_match("/sub([\d\w]{1,})/im", $res[$i+1], $m)) {
                $resarr['razdel'] = $res[$i];
                $resarr['sub'] = $m[1];
                $i += 2;
            } elseif (!empty($res[$i+1]) && preg_match("/^([\d]{1,})$/im", $res[$i+1])) {
                $resarr['razdel'] = $res[$i];
                $resarr['object'] = $res[$i+1];
                $i += 2;
            } elseif (!empty($res[$i+1]) && preg_match("/^arhiv$/im", $res[$i+1])) {
                $resarr['razdel'] = $res[$i];
                $resarr['arhiv'] = '';
                $i += 2;
            }
