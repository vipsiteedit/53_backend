...
function SE_PARTSELECTOR($razdel, $count, $limit, $item, $sel)
{
	...
	if (!empty($_SESSION['SE'])) {
       ...
       $item = 1; $sel = '';
	list($sect_id, $item ) = explode('-',getRequest('item'));
    if($sect_id == $razdel){
	$_SESSION['SE'][$_page.'_'.$razdel.'_item'] = $item;
    ...
    if ($count > $limit)
    {
        ...
        if ($step > 0)
        {
            $links .= '<a class="fnts" id="Beg" href="' . $link . '?item='.$razdel.'-1' . $sel . '#sm' . $razdel . '">1</a>'
                      . '&nbsp;&nbsp;'
                      . '<a class="fnts" id="Back" href="' . $link . '?item=' . $razdel.'-'.(($step - 1) * 10 + 1) . $sel
                      . '#sm' . $razdel . '">&nbsp;...</a>'
                      . '&nbsp;';
        }
        ...
        while ($j <= $listcount)
        {
            ...
            if ($j == $item)
                ...
                $links .= '<a class="links" href="' . $link . '?item=' . $razdel.'-'.$j . $sel . '#sm' . $razdel . '">' . $j . '</a>&nbsp;';
            $j++;
        }
        ...
        {
            $links .= '&nbsp;'
                      . '<a class="fnts" id="Next" href="' . $link . '?item=' . $razdel.'-'.(($step + 1) * 10 + 1) . $sel
                      . '#sm' . $razdel . '">...&nbsp;</a>'
                      . '&nbsp;&nbsp;'
                      . '<a class="fnts" id="End" href="' . $link . '?item=' . $razdel.'-'.$listcount . $sel . '#sm' . $razdel . '">'.$listcount.'</a>';
        }
...
function seLogotype($logo)
{
    ...
    {
        $imgs = '<div id="siteLogotype" onclick="location.href=\'/\';"'
                . '>' . setimages($fimg, 0, 1)
                . '</div>';
    }
    else
        $imgs = '<a href="/"><img id="siteLogotype" src="' . $ss[0] . '" border="0" alt="' . $title . '"></a>';
