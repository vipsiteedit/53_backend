...
class seData
{
    ...
    public function execute()
    {
        ...
        if (!empty($this->sections)) {
            foreach ($this->sections as $id_content => $section) {
                ...
                //if (!in_array($this->getNumContainer($id_content), $this->contarr, false)) continue;
    ...
    private function getNumContainer($sectionId)
    {
        if (strpos($sectionId,'.')!== false)
            list($sectionId,) = explode('.', $sectionId);
        if ($sectionId > 1000)
            return floor($sectionId / 1000);
        else 
            return 0;
    }
    ...
    {
        return ($name == strval($this->startpage)) ? seMultiDir() . URL_END : seMultiDir() . '/' . $name . URL_END;
    }
    ...
    public function getFolderModule($type)
    {
        ...
        if (file_exists(getcwd() . $syspath  . $path . '/module_' . $type . '.class.php')
            || file_exists(getcwd() . $syspath  . $path . '/mdl_' . $type . '.php')
        ) {
            return  $syspath . $path;
        } elseif (file_exists(getcwd() . $pathalt . $path . '/module_' . $type . '.class.php')
            || file_exists(getcwd() . $pathalt . $path . '/mdl_' . $type . '.php')
        ) {
            return $pathalt . $path;
        } elseif (file_exists(getcwd() . $path . '/module_' . $type . '.class.php')
    ...
    public function getPathLinks($space = '/', $endtitle = 'Home')
    {
        ...
        foreach($this->pages as $page) {
            if (strval($page['name']) == $startpage) {
                $link = ' <span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a href="'.seMultiDir().'/" itemprop="item"><span itemprop="name">' . $page->title . '</span></a><meta itemprop="position" content="'.++$pos.'"></span> ';
                break;
            }
        }
        ...
        if ($this->req->razdel && $this->req->object) {
            ...
            foreach($objects as $object) {
                if ($object->id == intval($this->req->object)) {
                    $linkTemplate = '<span class="space">'. $space . '</span> '.'<span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><span class="endtitle" itemprop="name">'.$object->title.'</span>';
                    break;
                }
            }
        ...
        foreach($level_arr as $line) {
            ...
            if (($linkTemplate == '') && ($getLastElement['title'] == $line['title'])) {
                $link .= '<span class="space">'.$space . '</span> <span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><span itemprop="name">'.$line['title'].'</span><meta itemprop="position" content="'.++$pos.'"></span> ';
            } else {
                $link .= '<span class="space">'.$space . '</span> <span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a href="' . seMultiDir().'/'. $line['name'] . SE_END . '" itemprop="item"><span itemprop="name">'.$line['title'].'</span></a><meta itemprop="position" content="'.++$pos.'"></span> ';
            }
        }
        
        $link .= $linkTemplate ? ($linkTemplate . '<meta itemprop="position" content="'.++$pos.'"></span>') : '';
        
        if ($endtitle != ''){
            $link .= '<span class="space">'.$space . '</span> <span itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><span class="endtitle" itemprop="name">'. $endtitle .'</span><meta itemprop="position" content="'.++$pos.'"></span>';
        }
