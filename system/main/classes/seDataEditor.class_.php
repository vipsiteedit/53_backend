...
// ###############################  EDITORS METHODS ###############################
  ...
  private function setSection($id_section, $type = 'mtext'){
		...
		if (isset($_POST['parttitle'])){
			$section->title = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['parttitle']));
		}
		if (isset($_POST['parttype'])){
			$section->type = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['parttype']));
		}
		if (isset($_POST['parttext'])){
			$section->text = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['parttext']));
		}
		...
		if (isset($_POST['recid'])){
			...
			if (isset($_POST['recfield'])){
				$record->field = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['recfield']));
			}
			if (isset($_POST['rectitle'])){
				$record->title = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['rectitle']));
			}
			if (isset($_POST['recnote'])){
				$record->note = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['recnote']));
			}
			if (isset($_POST['rectext'])){
				$record->text = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['rectext']));
			}
			for ($i=1; $i<7; $i++)
				if (isset($_POST['rectext'.$i])){
					$f = 'text'.$i;
					$record->$f = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['rectext'.$i]));
				}
		} 
  ...
  private function addRecord($section) {
	...
	if ($rec_insert){
		...
		simplexml_insert_before($insert, $section->objects[0]);
		$record = $this->data->getObject($section, $id_object);
	} else {
  ...
  private function updatePage(){
        ...
        if (isset($_POST['enteringtext'])){
            $this->data->page->enteringtext = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['enteringtext']));
        }
        if (isset($_POST['closingtext'])){
            $this->data->page->closingtext = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['closingtext']));
        }
  ...
  private function AddStorePage(){
	if (isset($_POST['namepage']) && preg_match("/^[a-z0-9\-]+$/", $_POST['namepage'])) {
		...
		// Создаем страницу
			...
			$pages['name'] = getRequest('namepage');
			$pages->title = $title;;
			$pages->skin = 'default';
			$oldpage = $newpage;
  ...
  private function setMenus($items, $oldpage, $newpage, $title, $url = ''){
	foreach($items as $menu){
		if (strval($menu['name']) == $oldpage){
			...
			$this->setMenus($menu->item, $oldpage, $newpage, $title);
		}
	}
  }
*/  
	...
	if (isset($_GET['jqueryform']) && $_GET['jqueryform'] == 'sitevars'){
		...
		if (!empty($namefield) && !empty($typefield)){
			if ($namefield == 'enteringtext' || $namefield == 'closingtext' || $namefield == 'title'){
				$this->data->page->$namefield = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['newvalue']));
			} else {
				$this->data->$typefield->vars->$namefield = str_replace('"/'.SE_DIR, '"', stripslashes($_POST['newvalue']));
			}
			if ($typefield != 'prj'){
				$this->data->savePage();
			} else {
...
public function getBtnGroup($buttons, $add) {
  ...
  public function getModules(){
	$groups = array();
	if (file_exists(SE_ROOT.'/system/service.xml'))
	{
