...
class seData {
  ...
  {
  	  return ($this->editorAccess() && file_exists(SE_ROOT.'projects/' . SE_DIR . 'edit/' . $namefile)
  ...
  private function initprj()
  {
        ...
        if (file_exists(SE_ROOT.'projects/' . SE_DIR . $folder . 'project.xml')) {
            ...
            if (strval($this->prj->sitedomain) && $this->prj->siteredirect == '1') {
                ...
                if ($_SERVER['HTTP_HOST'] != $urlsite && $urlsite) {
                    $this->go301('http://' . $urlsite . $_SERVER['REQUEST_URI']);
                }
            }
        ...
        if (!empty($this->prj->setools)) {
            ...
            $this->header[] = "<link rel=\"stylesheet\" type=\"text/css\" href=\"/lib/js/fancybox2/jquery.fancybox.css\">";
  ...
  private function initpage() {
		$folder = $this->getWorkFolder('pages/' . $this->pagename . '.xml');
		if (!file_exists(SE_ROOT.'projects/' . SE_DIR . $folder . 'pages/' . $this->pagename  .'.xml')) {
			$lastmodif = filemtime(SE_ROOT.'projects/' . SE_DIR . $folder . 'pages/' . $this->pagename  .'.xml');
		    $this->error = true;

	        if (!file_exists('projects/' . SE_DIR . 'pages/404.xml')) 
			{
	  			header('HTTP/1.0 404 File not found');
	  			header("Location: http://e-stile.ru/404.html");
	  			exit();
	  		} 
			else 
			{
...
//      echo SE_ROOT.'projects/' . SE_DIR . $folder . 'pagemenu.xml' . "<br/>";
...
/*	if ($this->services != null)
  ...
  public function getHeader($text, $section = '')
  {
	...
	while ($modulefolder && preg_match("/\[include_css\]/imu", $text, $m)){
		$cssfolder = $this->getSkinService() . '/' . strval($section->type);
        if(!file_exists($cssfolder .'/style.css')) {
		   $cssfolder = $modulefolder . '/css';
		}
        $text = str_replace($m[0], '<link href="/'.$cssfolder.'/style.css" rel="stylesheet" type="text/css">', $text);
    }
	while ($modulefolder && preg_match("/\[module_js:([^\]]*)\]/imu", $text, $m)){
		$jsfolder = $this->getSkinService() . '/' . strval($section->type);
        if(!file_exists($cssfolder .'/style.css')) {
		   $jsfolder = $modulefolder;
		}		
		$s1 = "\r\n<script type=\"text/javascript\" src=\"/{$jsfolder}/{$m[1]}\"></script>";
        $text = str_replace($m[0], $s1, $text);
    }
  ...
  public function getVars($type, $name){
	...
	{
		$result = htmlspecialchars($this->page->$name);
	} else {
...
//         $text = '<div class="se-section-block" data-event-dbl="frame_edit" data-subject="section" data-target="frame" data-id="'.$section->id.'"'.$records.'>'.$text.'</div>';
     }
     return $text;
  }
 ...
 // Методы для работы с редактором 
  ...
  private function conditions($text) {
       ...
       || preg_match("/\<if:\s?([^\>]+)\s?\>/im", $text, $m)) {
           $m[1] = str_replace('[thispage.link]', 'seMultiDir().\'/\' . $__data->getPageName() . \'/\'', $m[1]);
  ...
  public function parseModule($tpl, $section) {
       ...
       while (preg_match("/<repeat:pages>(.+?)<\/repeat:pages>/imu", $tpl, $m))
       {
       ...
       $tpl = str_replace('[thispage.link]', '<?php echo seMultiDir().\'/\' . $__data->getPageName() . \'/\' ?>', $tpl);
