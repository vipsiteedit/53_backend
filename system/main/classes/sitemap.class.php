...
class siteMap
{
    ...
    private function map_page($dir, $name, $folder, $lang = 'rus', $domain, $prior, $index = 1)
    {
        ...
        foreach ($page_data->sections as $section) {
            ...
            if (in_array($section->type, array('monlinenews', 'anews_public', 'apublication'))) {
                
                $ident = (string)$section->parametrs->param20;
                
                if (empty($ident) && $section->type != 'apublication') {
                    $ident = 'news';
                }
                
                $t = new seTable('news', 'n');
                $t->select("n.id, n.news_date, n.title, n.url");
                $t->innerJoin('news_category nc', 'nc.id=n.id_category');
                $t->andwhere("n.active='Y'");
                if ($ident) {
                    $t->where('nc.ident="?"', $ident);
                }
                $t->orderby('n.news_date', 1);
                ...
                if (!empty($newslist)) {
                    foreach ($newslist as $news) {
                        if ($section->type == 'apublication') {
                            $loc = $domain . '/' . $folder . $name . '/' . $news['url'];
                        }  else {
                            $loc = $domain . '/' . $folder .  'show' . $ident . '/' . $news['id'];
                        }
                        
                        $lastmod = date("c", ($news['news_date']));
                        $changefreq = $this->changefreq(($news['news_date']));
                        $this->arr[] = array('loc' => $loc, 'lastmod' => $lastmod, 'changefreq' => $changefreq, 'priority' => $prior);
            ...
            if (SE_DB_ENABLE && $index) {
                if (strpos($section->type, 'shop_groups') !== false || strpos($section->type, 'shop_vitrine') !== false) {
                    ...
                    foreach ($groups as $key => $code) {
                        ...
                        $loc = $domain . '/' . $folder . $name . "/cat/" . $code['code'];
