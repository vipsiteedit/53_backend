...
mysql_query("DROP TABLE IF EXISTS `shop_group_feature`;");
...
mysql_query("ALTER TABLE `shop_group_feature`
...
$req = mysql_query("SELECT  DISTINCT id_mod_group, CONCAT_WS(  '', GROUP_CONCAT(  `id_feature` ) ) AS features
...
//print_r($res);
  ...
  for ($i = count($features)-1; $i>=0; $i--){
     $it = $features[$i];
     mysql_query("INSERT INTO `shop_group_feature` (`id_group`,`id_feature`) VALUES ('{$id_group}','{$it}')");
  }
}
//echo mysql_error();